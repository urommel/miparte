            ...
            <tbody>
                @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as $dia)
                    <tr>
                        <td>{{ $dia }}</td>
                        <td>
                            <input type="checkbox" name="horarios[{{ strtolower($dia) }}][trabaja]" value="1" {{ $personal->horario->horarios[strtolower($dia)]['trabaja'] ? 'checked' : '' }}>
                        </td>
                        <td>
                            <input type="time" name="horarios[{{ strtolower($dia) }}][hora_inicio]" value="{{ $personal->horario->horarios[strtolower($dia)]['hora_inicio'] ?? '' }}">
                        </td>
                        <td>
                            <input type="time" name="horarios[{{ strtolower($dia) }}][hora_fin]" value="{{ $personal->horario->horarios[strtolower($dia)]['hora_fin'] ?? '' }}">
                        </td>
                    </tr>
                @endforeach
            </tbody>
        ...
        <tbody>
            @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as $dia)
                <tr>
                    <td>{{ $dia }}</td>
                    <td>
                        {!! Form::checkbox("horarios[$dia][trabaja]", 1, isset($personal->horario) ? $personal->horario->horarios[$dia]['trabaja'] : false) !!}
                    </td>
                    <td>
                        {!! Form::text("horarios[$dia][hora_inicio]", isset($personal->horario) ? $personal->horario->horarios[$dia]['hora_inicio'] : '', ['class' => 'form-control']) !!}
                    </td>
                    <td>
                        {!! Form::text("horarios[$dia][hora_fin]", isset($personal->horario) ? $personal->horario->horarios[$dia]['hora_fin'] : '', ['class' => 'form-control']) !!}
                    </td>
                </tr>
            @endforeach
        </tbody>
