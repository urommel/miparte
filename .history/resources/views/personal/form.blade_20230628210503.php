<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <div class="form-group">
            {{ Form::label('name') }}
            {{ Form::text('name', $personal->user()->first()->name, ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : ''), 'placeholder' => 'Name']) }}
            {!! $errors->first('name', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('email') }}
            {{ Form::text('email', $personal->user()->first()->email, ['class' => 'form-control' . ($errors->has('email') ? ' is-invalid' : ''), 'placeholder' => 'Email']) }}
            {!! $errors->first('email', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        Password y confirmar password
        <div class="form-group">
            {{ Form::label('password') }}
            {{ Form::text('password', $personal->user()->first()->password, ['class' => 'form-control' . ($errors->has('password') ? ' is-invalid' : ''), 'placeholder' => 'Password']) }}
            {!! $errors->first('password', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('password_confirmation') }}
            {{ Form::text('password_confirmation', $personal->user()->first()->password_confirmation, ['class' => 'form-control' . ($errors->has('password_confirmation') ? ' is-invalid' : ''), 'placeholder' => 'Password Confirmation']) }}
